 ...
 */ var SkeletonViewer = /*#__PURE__*/ function(Script) {
    ...
    function SkeletonViewer(entity) {
        ...
        /** The min color.  */ _this.colorMin = new miniprogram.Color(0.35, 0.35, 0.35, 1);
        /** The max color. */ _this.colorMax = new miniprogram.Color(0.7, 0.7, 0.7, 1);
        _this._debugMesh = [];
        var engine = entity.engine;
        if (!materialMap.get(engine)) {
            var material = new miniprogram.Material(entity.engine, miniprogram.Shader.find("skeleton-viewer"));
            material.renderState.rasterState.depthBias = -100000000;
            material.renderState.renderQueueType = miniprogram.RenderQueueType.Transparent;
            materialMap.set(engine, material);
        }
        _this._material = materialMap.get(engine);
        ...
        _this.entity.getComponentsIncludeChildren(miniprogram.SkinnedMeshRenderer, skinnedMeshRenderers);
    ...
    _proto._createSpur = function _createSpur(direction) {
        var mesh = new miniprogram.ModelMesh(this.engine);
        ...
        var matrix = new miniprogram.Matrix();
        var quaternion = new miniprogram.Quaternion();
        miniprogram.Matrix.lookAt(new miniprogram.Vector3(0, 0, 0), direction, new miniprogram.Vector3(0, 1, 0), matrix);
        matrix.getRotation(quaternion).invert();
        var vertex = [
            new miniprogram.Vector3(midHalfWidth, -midHalfWidth, -midLength).transformByQuat(quaternion),
            new miniprogram.Vector3(midHalfWidth, midHalfWidth, -midLength).transformByQuat(quaternion),
            new miniprogram.Vector3(-midHalfWidth, midHalfWidth, -midLength).transformByQuat(quaternion),
            new miniprogram.Vector3(-midHalfWidth, -midHalfWidth, -midLength).transformByQuat(quaternion),
            new miniprogram.Vector3(0, 0, -length).transformByQuat(quaternion),
            new miniprogram.Vector3(0, 0, 0)
        ];
        positions[0] = new miniprogram.Vector3(0, 0, 0);
        positions[1] = vertex[0].clone();
        positions[2] = vertex[1].clone();
        positions[3] = new miniprogram.Vector3(0, 0, 0);
        positions[4] = vertex[1].clone();
        positions[5] = vertex[2].clone();
        positions[6] = new miniprogram.Vector3(0, 0, 0);
        positions[7] = vertex[2].clone();
        positions[8] = vertex[3].clone();
        positions[9] = new miniprogram.Vector3(0, 0, 0);
        ...
        normals[0] = normals[1] = normals[2] = new miniprogram.Vector3(1, 0, 1).transformByQuat(quaternion);
        normals[3] = normals[4] = normals[5] = new miniprogram.Vector3(0, 1, 1).transformByQuat(quaternion);
        normals[6] = normals[7] = normals[8] = new miniprogram.Vector3(-1, 0, 1).transformByQuat(quaternion);
        normals[9] = normals[10] = normals[11] = new miniprogram.Vector3(0, -1, 1).transformByQuat(quaternion);
        normals[12] = normals[13] = normals[14] = new miniprogram.Vector3(1, 0, -1).transformByQuat(quaternion);
        normals[15] = normals[16] = normals[17] = new miniprogram.Vector3(0, 1, -1).transformByQuat(quaternion);
        normals[18] = normals[19] = normals[20] = new miniprogram.Vector3(-1, 0, -1).transformByQuat(quaternion);
        normals[21] = normals[22] = normals[23] = new miniprogram.Vector3(0, -1, -1).transformByQuat(quaternion);
    ...
    _proto._showSkeleton = function _showSkeleton(renderer) {
        ...
        for(var i = 0; i < bones.length; i++){
            ...
            var _$renderer = entity.addComponent(miniprogram.MeshRenderer);
            _$renderer.receiveShadows = false;
            _$renderer.castShadows = false;
            _$renderer.mesh = miniprogram.PrimitiveMesh.createSphere(this.engine, this.ballSize, 16);
            ...
            for(var j = 0; j < bone.children.length; j++){
                ...
                var entity1 = bone;
                var _$renderer1 = entity1.addComponent(miniprogram.MeshRenderer);
