...
var PlaneMesh = /*#__PURE__*/ function() {
    function PlaneMesh() {}
    PlaneMesh.createPlane = function createPlane(engine$1) {
    ...
    PlaneMesh._initGeometryData = function _initGeometryData(mesh) {
        ...
        positions[0] = new engine.Vector3(-1, -1, 0);
        positions[1] = new engine.Vector3(1, -1, 0);
        positions[2] = new engine.Vector3(1, 1, 0);
        positions[3] = new engine.Vector3(-1, 1, 0);
...
var BoxSelectionSSMaterial = /*#__PURE__*/ function(BaseMaterial) {
    ...
    function BoxSelectionSSMaterial(engine$1) {
        var _this;
        _this = BaseMaterial.call(this, engine$1, engine.Shader.find("box")) || this;
        _this.isTransparent = true;
        _this.boxColor = new engine.Vector4(0.29, 0.63, 1, 0.3);
        _this.borderColor = new engine.Vector4(0.22, 0.48, 1, 0.9);
...
BoxSelectionSSMaterial._borderWidth = engine.ShaderProperty.getByName("u_width");
BoxSelectionSSMaterial._minPoint = engine.ShaderProperty.getByName("u_min");
BoxSelectionSSMaterial._maxPoint = engine.ShaderProperty.getByName("u_max");
BoxSelectionSSMaterial._boxColor = engine.ShaderProperty.getByName("u_boxColor");
BoxSelectionSSMaterial._borderColor = engine.ShaderProperty.getByName("u_borderColor");
engine.Shader.create("box", "\n#include <common>\n#include <common_vert>\n\nvoid main() {\n  gl_Position = vec4(POSITION, 1.0);\n}", "\nuniform vec2 u_min;\nuniform vec2 u_max;\nuniform vec4 u_boxColor;\nuniform vec4 u_borderColor;\nuniform float u_width;\n\nvoid main() {\n  float vColor = step(u_min.x + u_width, gl_FragCoord.x) * step(gl_FragCoord.x, u_max.x - u_width) * step(u_min.y + u_width, gl_FragCoord.y) * step(gl_FragCoord.y, u_max.y - u_width);\n  float vBorder = step(u_min.x, gl_FragCoord.x) * step(gl_FragCoord.x, u_max.x) * step(u_min.y, gl_FragCoord.y) * step(gl_FragCoord.y, u_max.y);\n  gl_FragColor = u_boxColor * vColor + (1. - vColor) * vBorder * u_borderColor;\n}\n");

// draw according to screen space
var BoxSelectionSSHelper = /*#__PURE__*/ function() {
    function BoxSelectionSSHelper(engine$1, sceneRoot) {
        this.engine = engine$1;
        this.element = new engine.Entity(engine$1);
        this.sceneRoot = sceneRoot;
        this.startPoint = new engine.Vector2();
        this.pointBottomLeft = new engine.Vector2();
        this.pointTopRight = new engine.Vector2();
        this.input = engine$1.inputManager;
        this.init();
    }
    var _proto = BoxSelectionSSHelper.prototype;
    _proto.init = function init() {
        var renderer = this.element.addComponent(engine.MeshRenderer);
...
var _frustum = new engine.BoundingFrustum();
var _center = new engine.Vector3();
var _tmpPoint = new engine.Vector2();
var _vecNear = new engine.Vector3();
var _vecTopLeft = new engine.Vector3();
var _vecTopRight = new engine.Vector3();
var _vecDownRight = new engine.Vector3();
var _vecDownLeft = new engine.Vector3();
var _vecFarTopLeft = new engine.Vector3();
var _vecFarTopRight = new engine.Vector3();
var _vecFarDownRight = new engine.Vector3();
var _vecFarDownLeft = new engine.Vector3();
var _vectemp1 = new engine.Vector3();
var _vectemp2 = new engine.Vector3();
var _vectemp3 = new engine.Vector3();
var pojectInvertMatrix = new engine.Matrix();
function unproject(vec, entity) {
    engine.Vector3.transformCoordinate(vec, pojectInvertMatrix, vec);
    return engine.Vector3.transformCoordinate(vec, entity.transform.worldMatrix, vec);
}
var BoxSelectionControls = /*#__PURE__*/ function(Script) {
    ...
    function BoxSelectionControls() {
        ...
        _this.startPoint = new engine.Vector2();
        _this.endPoint = new engine.Vector2();
        _this.collection = [];
        _this.instances = {};
    ...
    _proto.onUpdate = function onUpdate() {
        ...
        if (this.input.isPointerDown(engine.PointerButton.Primary)) {
            var _this_helper;
        ...
        if (this.input.isPointerUp(engine.PointerButton.Primary)) {
            var _this_helper1;
        ...
        if (this.input.isPointerHeldDown(engine.PointerButton.Primary)) {
            var _this_helper2;
    ...
    _proto.updateBoundingFrustum = function updateBoundingFrustum(startPoint, endPoint) {
        ...
        if (!this.camera.isOrthographic) {
            ...
            engine.Plane.fromPoints(_vecNear, _vecTopLeft, _vecTopRight, _frustum.top);
            engine.Plane.fromPoints(_vecNear, _vecTopRight, _vecDownRight, _frustum.right);
            engine.Plane.fromPoints(_vecDownRight, _vecDownLeft, _vecNear, _frustum.bottom);
            engine.Plane.fromPoints(_vecDownLeft, _vecTopLeft, _vecNear, _frustum.left);
            engine.Plane.fromPoints(_vecTopRight, _vecDownRight, _vecDownLeft, _frustum.near);
            engine.Plane.fromPoints(_vectemp3, _vectemp2, _vectemp1, _frustum.far);
            ...
            engine.Plane.fromPoints(_vecTopLeft, _vecFarTopLeft, _vecFarTopRight, _frustum.top);
            engine.Plane.fromPoints(_vecTopRight, _vecFarTopRight, _vecFarDownRight, _frustum.right);
            engine.Plane.fromPoints(_vecFarDownRight, _vecFarDownLeft, _vecDownLeft, _frustum.bottom);
            engine.Plane.fromPoints(_vecFarDownLeft, _vecFarTopLeft, _vecTopLeft, _frustum.left);
            engine.Plane.fromPoints(_vecTopRight, _vecDownRight, _vecDownLeft, _frustum.near);
            engine.Plane.fromPoints(_vecFarDownRight, _vecFarTopRight, _vecFarTopLeft, _frustum.far);
            _frustum.far.normal.scale(-1);
        }
    };
    _proto.searchChildInBoundingFrustum = function searchChildInBoundingFrustum(frustum, entity, isDeep) {
        ...
        if (c && c.enabled) {
            ...
            c.isSelect = engine.CollisionUtil.frustumContainsPoint(frustum, _center) !== engine.ContainmentType.Disjoint;
            if (c.isSelect) {
