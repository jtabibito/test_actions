...
exports.OutlineManager = /*#__PURE__*/ function(Script) {
    ...
    function OutlineManager(entity) {
        ...
        _this._size = 1;
        _this._clearColor = new miniprogram.Color(1, 1, 1, 1);
        _this._replaceColor = new miniprogram.Color(1, 0, 0, 1);
        _this._outlineMainColor = new miniprogram.Color(0.95, 0.35, 0.14, 1);
        _this._outlineSubColor = new miniprogram.Color(0.16, 0.67, 0.89, 1);
        _this._layer = miniprogram.Layer.Layer29;
        _this._outlineEntities = [];
        _this._subLineEntities = [];
        _this._renderers = [];
        _this._layerMap = [];
        _this._cameraViewport = new miniprogram.Vector4();
        _this._outLineViewport = new miniprogram.Vector4(0, 0, 1, 1);
        var engine = _this.engine;
        var outlineMaterial = new miniprogram.BaseMaterial(engine, miniprogram.Shader.find("outline-postprocess-shader"));
        var replaceShader = miniprogram.Shader.find("outline-replace-shader");
        var screenEntity = _this.entity.createChild("screen");
        var screenRenderer = screenEntity.addComponent(miniprogram.MeshRenderer);
        ...
        screenRenderer.mesh = miniprogram.PrimitiveMesh.createPlane(engine, 2, 2);
   ...
   */ _proto.removeEntity = function removeEntity(entity) {
    ...
    _proto._renderEntity = function _renderEntity(camera, outlineColor, entities) {
        ...
        for(var i = entities.length - 1; i >= 0; i--){
            ...
            // replace material
            renderers.length = 0;
            entity.getComponents(miniprogram.MeshRenderer, renderers);
            // replace layer
            if (renderers.length) {
                layerMap.push({
                    entity: entity,
        ...
        scene.background.mode = miniprogram.BackgroundMode.SolidColor;
        ...
        camera.clearFlags = miniprogram.CameraClearFlags.None;
    ...
    _proto._checkFrameBufferSize = function _checkFrameBufferSize(size) {
        ...
        if (!this._renderTarget || size !== lastSize || this._renderTarget.width !== offWidth || this._renderTarget.height !== offHeight) {
            ...
            var renderColorTexture = new miniprogram.Texture2D(this.engine, offWidth, offHeight);
            var renderTarget = new miniprogram.RenderTarget(this.engine, offWidth, offHeight, renderColorTexture);
            this._outlineMaterial.shaderData.setTexture(OutlineManager._outlineTextureProp, renderColorTexture);
            this._outlineMaterial.shaderData.setVector2(OutlineManager._texSizeProp, new miniprogram.Vector2(1 / offWidth, 1 / offHeight));
            renderColorTexture.wrapModeU = renderColorTexture.wrapModeV = miniprogram.TextureWrapMode.Clamp;
...
exports.OutlineManager._outlineColorProp = miniprogram.ShaderProperty.getByName("material_OutlineColor");
exports.OutlineManager._outlineTextureProp = miniprogram.ShaderProperty.getByName("material_OutlineTexture");
exports.OutlineManager._texSizeProp = miniprogram.ShaderProperty.getByName("material_TexSize");
exports.OutlineManager._replaceColorProp = miniprogram.ShaderProperty.getByName("camera_OutlineReplaceColor");
...
miniprogram.Shader.create("outline-postprocess-shader", outlineVs, outlineFs);
miniprogram.Shader.create("outline-replace-shader", replaceVs, replaceFs);
