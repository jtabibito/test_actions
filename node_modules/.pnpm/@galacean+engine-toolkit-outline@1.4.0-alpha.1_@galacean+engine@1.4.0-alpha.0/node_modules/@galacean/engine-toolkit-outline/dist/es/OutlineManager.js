import { ShaderProperty, dependentComponents, Camera, DependentMode, Shader, MeshRenderer, BackgroundMode, DepthTextureMode, CameraClearFlags, Texture2D, RenderTarget, Vector2, TextureWrapMode, Color, Layer, Vector4, BaseMaterial, PrimitiveMesh, Script } from '@galacean/engine';
...
var OutlineManager = /*#__PURE__*/ function(Script) {
    ...
    function OutlineManager(entity) {
        ...
        _this._size = 1;
        _this._clearColor = new Color(1, 1, 1, 1);
        _this._replaceColor = new Color(1, 0, 0, 1);
        _this._outlineMainColor = new Color(0.95, 0.35, 0.14, 1);
        _this._outlineSubColor = new Color(0.16, 0.67, 0.89, 1);
        _this._layer = Layer.Layer29;
        _this._outlineEntities = [];
        _this._subLineEntities = [];
        _this._renderers = [];
        _this._layerMap = [];
        _this._cameraViewport = new Vector4();
        _this._outLineViewport = new Vector4(0, 0, 1, 1);
        var engine = _this.engine;
        var outlineMaterial = new BaseMaterial(engine, Shader.find("outline-postprocess-shader"));
        var replaceShader = Shader.find("outline-replace-shader");
        var screenEntity = _this.entity.createChild("screen");
        var screenRenderer = screenEntity.addComponent(MeshRenderer);
   ...
   */ _proto.removeEntity = function removeEntity(entity) {
    ...
    _proto._checkFrameBufferSize = function _checkFrameBufferSize(size) {
        ...
        if (!this._renderTarget || size !== lastSize || this._renderTarget.width !== offWidth || this._renderTarget.height !== offHeight) {
            ...
            var renderColorTexture = new Texture2D(this.engine, offWidth, offHeight);
            var renderTarget = new RenderTarget(this.engine, offWidth, offHeight, renderColorTexture);
...
OutlineManager._outlineColorProp = ShaderProperty.getByName("material_OutlineColor");
OutlineManager._outlineTextureProp = ShaderProperty.getByName("material_OutlineTexture");
OutlineManager._texSizeProp = ShaderProperty.getByName("material_TexSize");
OutlineManager._replaceColorProp = ShaderProperty.getByName("camera_OutlineReplaceColor");
OutlineManager = __decorate([
